...
class App {
  ...
  setupGeometry() {
    ...
    this.position = [
       ...
       -0.5, 0.15,  0.0, // ふたつ目の頂点の x, y, z 座標
      -0.25, -0.5,  0.0, // みっつ目の頂点の x, y, z 座標
    ...
    this.positionStar = [
       0.0,  0.5,  0.0, // ひとつ目の頂点の x, y, z 座標
       0.2, 0.0, 0.0, // ふたつ目の頂点の x, y, z 座標
      -0.4, -0.5,  0.0, // みっつ目の頂点の x, y, z 座標

      0.0,  0.5,  0.0, // ひとつ目の頂点の x, y, z 座標
      -0.2, 0.0, 0.0, // ふたつ目の頂点の x, y, z 座標
     0.4, -0.5,  0.0, // みっつ目の頂点の x, y, z 座標

      -0.5,  0.2,  0.0, // ひとつ目の頂点の x, y, z 座標
      0.0, -0.167,  0.0, // ふたつ目の頂点の x, y, z 座標
      0.5, 0.2,  0.0, // みっつ目の頂点の x, y, z 座標
    ];
    // 要素数は XYZ の３つ
    this.positionStrideStar = 3;
    // VBO を生成
    this.positionStarVBO = WebGLUtility.createVBO(this.gl, this.positionStar);
    ...
    this.color = [
      ...
      1.0, 0.0, 0.0, 1.0, // ひとつ目の頂点の r, g, b, a カラー
      0.0, 0.0, 1.0, 1.0, // ふたつ目の頂点の r, g, b, a カラー
      0.0, 0.0, 1.0, 1.0, // みっつ目の頂点の r, g, b, a カラー
    ];
    // 要素数は RGBA の４つ
    this.colorStride = 4;
  ...
  setupLocation() {
    ...
    const positionStarAttributeLocation = gl.getAttribLocation(this.program, 'positionStar');
    const colorAttributeLocation = gl.getAttribLocation(this.program, 'color');
    // WebGLUtility.enableBuffer は引数を配列で取る仕様なので、いったん配列に入れる
    const vboArray = [this.positionVBO, this.positionStarVBO, this.colorVBO];
    const attributeLocationArray = [positionAttributeLocation,positionStarAttributeLocation, colorAttributeLocation];
    const strideArray = [this.positionStride, this.positionStrideStar, this.colorStride];
    // 頂点情報の有効化
  ...
  render() {
    ...
    gl.uniform1f(this.uniformLocation.direction, this.directionObj.value);
